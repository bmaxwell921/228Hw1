...
public class Grader extends RunListener {
	 ...
	 * 
	 */
	public Grader(Class<?> testClass, String testName) {
		this.testClass = testClass;
		this.testName = testName;
	}

	public Grader(Class<?> testClass, String testName, PrintStream output)
			throws FileNotFoundException {
	...
	public void testFailure(Failure failure) throws Exception {
		...
		if (!(failure.getException() instanceof AssertionError)) {
			...
			StackTraceElement locThrown = failure.getException()
					.getStackTrace()[0];
			String fullLocation = locThrown.getFileName() + ":"
					+ locThrown.getLineNumber();
			message = "Threw " + name + exceptionMessage + " at ("
					+ fullLocation + ")";
		}
		System.out.printf("[%s] %s (-%d points)%n", penalty.getMethodName(),
				message, getPoints(penalty));
	}
	...
	public void testRunFinished(Result result) throws Exception {
		...
		System.out.println("***** "
				...
				+ " tests passed and " + result.getFailureCount()
				+ " failed out of " + result.getRunCount() + " total on "
				+ testName);
		double normalized = ((pointsPossible - pointsLost) * 1.0 / pointsPossible)
				* totalResult;

		System.out.println(String.format("      Raw Score: %d/%d",
				(pointsPossible - pointsLost), pointsPossible));
		System.out.println(String.format("      Normalized Score: %.2f/%d",
				normalized, totalResult));
	 ...
	 */
	private int getPoints(Description description) {
		Points annotation = description.getAnnotation(Points.class);
		if (annotation == null)
			System.err.println(description.getMethodName()
					+ " does not have a Points annotation!");
	...
	private int getClassTotal() {
		...
		if (annotation == null)
			System.err.println(testClass.getName()
					+ " does not have a Total annotation!");
		int value = annotation.value();
		return value;
	}
}
